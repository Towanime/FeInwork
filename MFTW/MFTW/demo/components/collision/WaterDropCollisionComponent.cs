...
namespace FeInwork.FeInwork.components
{
    ...
    /// </summary>
    public class WaterDropCollisionComponent : AbstractCollisionComponent, PositionChangeRequestListener, StateChangeListener, DeadListener
    {
        public WaterDropCollisionComponent(IEntity owner, CollisionBody shape)
            : base(owner, shape)
        {
            this.initialize();
        }
        ...
        public void invoke(PositionChangeRequestEvent eventObject)
        {
            ...
            for (int shapeIndex = 0; shapeIndex < bodyList.Count; shapeIndex++)
            {
                ...
                foreach (CollisionBody secondShape in shapeGroup)
                {
                    ...
                    if (secondShape.Owner.containsState(EntityState.WaterDrop)) continue;
